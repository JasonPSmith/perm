...
public class mob{
	public static void main(String[] args){		
		...
		int m = mob(sigma,pi);
		int occ = NumOccurrences(sigma,pi);
		int ne = NE(sigma,pi);
		int s = secondPart(sigma,pi,m,ne);
		ArrayList<String> lambdas = mobNEprint(sigma,pi);

		System.out.println();
		System.out.println("Mobius function = " + m);
		System.out.println("<br>");
		System.out.println("Number of Occurrences = " + occ);
		System.out.println("Number of Normal Occurrences/Embeddings = " + ne);
		System.out.println();
		System.out.println("Value of second part s = " + s);
		System.out.println();
		if(lambdas.size() > 0) System.out.println("Lambdas in [sigma,pi) with nonzero mu(sigma,lambda)*EZ(lambda,sigma):");
		for(int i = 0; i < lambdas.size(); i++) System.out.println(lambdas.get(i));
...
/***************************************************************/
	...
	//same as mobNE above but prints what each element of the formula contributes
	private static ArrayList<String> mobNEprint(int[] sigma, int[] pi){
		int[][] o = Occurrences(sigma,pi);
		int[][] occ = occQuotient(o,pi);
		int[] adjs = adjLocBoth(pi);
		int num = 0;


		//get all perms in the interval
		Interval I = new Interval(sigma,pi);

		//get all perms
		ArrayList<int[]> perms  = I.listAllPerms();

		int s,mob,temp;
		ArrayList<String> out = new ArrayList<String>();
		
		//compute EZ(sigma,pi), add to num and print if EZ is none empty
		String t = permString(perms.get(0));
		s = incExSum(perms.get(0),pi,occ);
		permString(perms.get(0));
		if(s!=0){t += " : Contributes = " + s +" : product of mu = 1 and EZ = " + s;out.add(t);}
		
		
		//loop through all elements of (sigma,pi)
		for(int i = 1; i< perms.size(); i++){
			t = "";
			if(pi.length-perms.get(i).length>0){
				//print lambda and compute EZ(lambda,pi)
				s = incExSum(perms.get(i),pi);
				if(!emptyEZ(perms.get(i),pi)){
					t += permString(perms.get(i));
					//if EZ is non empty compute mu(sigma,lambda) and print all
					mob = mob(sigma,perms.get(i));
					temp = s*mob;
					t +=" : Contributes = " + temp + " : product of mu = " + mob + " and EZ = " + s;
					if(temp!=0) out.add(t);
				}
			}
		}
	...
	//returns the second part of the formula
	private static int secondPart(int[] sigma, int[] pi,int mob,int NE){
		//subtract sNE(sigma,pi) from mu(sigma,pi)
		return (mob - (int)Math.pow(-1,pi.length-sigma.length)*NE);
	}
	
	//returns sum over EZ^{lambda,pi}
	private static int incExSum(int[] lambda, int[] pi){
		//get set of occurrences and tehen quotient them
		int[][] o = Occurrences(lambda,pi);
		int[][] occ = occQuotient(o,pi);
		int num = 0;

		//create a binary string which indicates which elements of occ to test if in EZ
		long m = (long)Math.pow(2,occ.length);
		int[] bin = new int[occ.length];
		int s;
	
		//intialise array as all 1's
		for(int j = 0; j < bin.length; j++) bin[j] = 1;
		//because EZ is upwards closed check if the set of all elemebts of occ are in EZ and if not then nothing is
		if(!emptyZeroes(bin,occ,pi.length)) return 0;

		//add (-1)^|S| for each S that has empty zero intersection, where S is displayed as a binary string relating to occ
		for(long i = m-1; i > 0; i--){
			bin = BinaryIntArray(i, occ.length);
			s = sum(bin);
			if(s!=1){
				if(emptyZeroes(bin,occ,pi.length)){
	...
	//returns true if EZ^{lambda,pi} is empty
	private static boolean emptyEZ(int[] lambda, int[] pi){
		int[][] o = Occurrences(lambda,pi);
		int[][] occ = occQuotient(o,pi);
		...
		for(int j = 0; j < bin.length; j++) bin[j] = 1;
		if(!emptyZeroes(bin,occ,pi.length)) return true;
		return false;
	
	}
	//returns true if EZ^{lambda,pi} is empty
	private static boolean emptyEZ(int[] lambda, int[] pi,int[][] occ){
		long m = (long)Math.pow(2,occ.length);
		int[] bin = new int[occ.length];
	...
	//returns sum over EZ^{lambda,pi}, same incExSum above to be used when occ is already known
	private static int incExSum(int[] lambda, int[] pi, int[][] occ){
		int num = 0;
	...
	private static ArrayList<int[]> EZ(int[] sigma,int[] pi){
		//get occurrences and quotient;
		int[][] o = Occurrences(sigma,pi);
		int[][] occ = occQuotient(o,pi);
		...
		long m = (long)Math.pow(2,occ.length);
		int[] bin = new int[occ.length];
		ArrayList<int[]> binList = new ArrayList<int[]>();
	
		//add any nonzero sets to binList
		for(long i = m-1; i > 0; i--){
			bin = BinaryIntArray(i, occ.length);
			if(sum(bin)!=1){
				if(emptyZeroes(bin,occ,pi.length)){
					binList.add(bin.clone());
				}
			}
		}
	...
	private static ArrayList<int[]> nonZeroEZ(int[] sigma,int[] pi){
		//get occurrences and quotient;
		int[][] o = Occurrences(sigma,pi);
		int[][] occ = occQuotient(o,pi);
	...
	private static void printPermln(int[] perm){
		printPerm(perm);
		System.out.println("");
	}
	...
	private static void printPermln(int[] perm,int[] pi){
		printPerm(perm);
		System.out.print(" : ");
		printPerm(pi);
		System.out.println("");
	...
	//converts the permutation to a string
	private static String permString(int[] a){	
		String  ret = "";
		for( int i = 0; i < a.length; i++) ret += (a[i]+1);
		return ret;
    }
	
/*********************************************/
//methods for reading a interval from input
	//read two permutations
	private static int[][] readPerm(String[] args){
		int[] param = new int[args.length];
    	for(int k = 0; k < args.length; k++) param[k] = Integer.parseInt(args[k]);
		int[] sigma = NumToPerm(param[0]); 
		int[] pi = PermOf(TailFrom(param, 1));
		
		int[][] out = new int[][]{sigma,pi};
		return out;
	}
...
//various methods needed for creating intervals
	...
	//removes all letters perm[start]-perm[end] inclusive and reduces
	private static int[] removeLetters(int[] perm, int start, int end){
		int[] newperm = new int[perm.length-(end-start+1)];

		System.arraycopy(perm,0,newperm,0,start);
		System.arraycopy(perm,end+1,newperm,start,newperm.length-start);

		return reduce(newperm);
	}
	//removes all letters at the positions in locs
	private static int[] removeLetters(int[] perm, int[] locs){
		int[] newperm = new int[perm.length-locs.length];
...
//various methods used for computing MF
	//sums the elements pi_start+...+pi_{end-1}
	private static int sum(int start, int end, int[] pi){
		int s = 0;
		for(int i = start; i < end; i++) s += Math.abs(pi[i]);
		return s;
	}
	//sums all elements of bin
	private static int sum(int[] bin){
		int s = 0;
		for(int i = 0; i < bin.length; i++) s+=bin[i];
		return s;
	}	
	//sums abs of all elements of bin
	private static int sumAbs(int[] bin){
		int s = 0;
		for(int i = 0; i < bin.length; i++) s+=Math.abs(bin[i]);
		return s;
	}
	...
	//if the set S indicated by binary string bin has empty zero set intersection return true
	private static boolean emptyZeroes(int[] bin, int[][] occ, int n){
		for(int i = 0; i < n; i++){
			boolean allZero = true;
			for(int j = 0; j < bin.length; j++){
				if(bin[j] == 1 && con(i,occ[j])) allZero = false;
			}
...
//various methods taken from e.java
	private static int[] NumToPerm(int number){

		int Dim;

		for(Dim = 0; number/((int)Math.pow(10,Dim)) > 0; Dim++);
	...
	private static int[] TailFrom(int[] A, int start){

		if(A == null)
		return null;

		int[] output = new int[A.length-start];
	...
	private static int[] PermOf(int[] param){

		int dim = param.length;;

		if(dim == 1)
		return StandardPerm(NumToPerm(param[0]));
	     
		return StandardPerm(param); 
	}
	
	private static int[] StandardPerm(int[] perm){
		if(perm == null)
			return null;
		return InvTableToPerm(InversionTable(perm));
	}
	
	private static int[] InvTableToPerm(int[] invTable){

		if(invTable == null || invTable.length == 0)
		return null;

		int dim = invTable.length;
		...
		for(int k = 0; k < dim; k++){
		perm[k] = (int)list.get(invTable[k]);
		list.remove(invTable[k]);
		}
